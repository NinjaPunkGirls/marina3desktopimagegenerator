...
document.addEventListener("DOMContentLoaded", () => {
    ...
    generateBtn.addEventListener("click", async () => {
        ...
        if (currentMode === 'general' && (currentGeneralSubMode === 'image-to-image' || currentGeneralSubMode === 'text-image-reference') && initImageFileObject) {
            ...
            const textPromptToSend = (currentGeneralSubMode === 'text-to-image' || (prompt && prompt.trim() !== '')) ? prompt.trim() : '(empty prompt)'; // Use user prompt or a default non-empty string

            // Only append text_prompts if not in pure image-to-image mode with an empty prompt
            if (!(currentGeneralSubMode === 'image-to-image' && (!prompt || prompt.trim() === '' ))) {
