...
document.addEventListener("DOMContentLoaded", () => {
    ...
    singleImageInput.addEventListener('change', (event) => {
        ...
        if (file) {
            ...
            reader.onload = (e) => {
                // Clear previous images in the grid and set the selected single image
                imageGrid.innerHTML = '';
                // When selecting a single image, we should not keep previously selected images from folder
                if (selectedInitImage && selectedInitImage.parentElement === imageGrid) {
                     imageGrid.removeChild(selectedInitImage);
                }
                selectedInitImage = null; // Clear previous selected image
                ...
                selectedInitImage = imgElement; // Set the newly selected single image
    ...
    ipcRenderer.on('selected-directory', (event, directoryPath) => {
        if (directoryPath) {
            ...
            // Clear any previously selected single image when selecting a folder
            if (selectedInitImage && selectedInitImage.parentElement === imageGrid) {
                imageGrid.removeChild(selectedInitImage);
            }
            selectedInitImage = null;
    ...
    generateBtn.addEventListener("click", async () => {
        ...
        if (currentMode === 'general' && (currentGeneralSubMode === 'image-to-image' || currentGeneralSubMode === 'text-image-reference') && initImageFileObject) {
            ...
            // Include text prompts in Image to Image mode (with a default if empty) and Text and Image Reference mode
            const textPromptToSend = prompt && prompt.trim() !== '' ? prompt.trim() : '(empty prompt)'; // Use a default non-empty prompt if user prompt is empty
            ...
            requestBody = JSON.stringify({
             ...
             if (selectedInitImage) {
                 ...
                 formData.append('text_prompts[0][text]', posterPrompt); // Include the constructed prompt
